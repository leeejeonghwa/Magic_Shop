...
public class Mypage_ChangePasswordActivity extends AppCompatActivity {
        ...
        btn_change.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!user_new_pass.equals(user_new_pass_check)) {
                    ...
                    Toast.makeText(Mypage_ChangePasswordActivity.this, "새로운 비밀번호가 일치하지 않습니다.", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (pass.equals(userPassword)) {
                    ...
                    // 기존 비밀번호 확인 실패
                    Toast.makeText(Mypage_ChangePasswordActivity.this, "기존 비밀번호가 일치하지 않습니다.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    private void sendChangePasswordRequest(String userID, String newPassword) {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (success) {
                        ...
                        Toast.makeText(Mypage_ChangePasswordActivity.this, "비밀번호가 성공적으로 변경되었습니다.", Toast.LENGTH_SHORT).show();
                        // 변경 성공 후 다음 화면으로 이동하는 코드 추가
                        Intent intent = new Intent(Mypage_ChangePasswordActivity.this, Mypage_SettingActivity.class);
                        startActivity(intent);
                    } else {
                        // 변경 실패에 대한 처리
                        Log.d("Mypage_ChangePassword", "비밀번호 변경 실패");
                        Toast.makeText(Mypage_ChangePasswordActivity.this, "비밀번호 변경에 실패하였습니다.", Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                    // JSON 파싱 오류가 발생한 경우
                    Log.e("Mypage_ChangePassword", "서버 응답 파싱 오류: " + response);
                    Toast.makeText(Mypage_ChangePasswordActivity.this, "서버 응답을 처리하는 중 오류가 발생했습니다.", Toast.LENGTH_SHORT).show();
                }
            }
        };
        ...
        RequestQueue changePasswordQueue = Volley.newRequestQueue(Mypage_ChangePasswordActivity.this);
